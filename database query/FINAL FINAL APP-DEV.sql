...
USE `db_appdev_updated`$$
DROP TRIGGER IF EXISTS `db_appdev_updated`.`purchase_order_BEFORE_INSERT` $$
USE `db_appdev_updated`$$
CREATE DEFINER = CURRENT_USER TRIGGER `db_appdev_updated`.`purchase_order_BEFORE_INSERT` BEFORE INSERT ON `purchase_order` FOR EACH ROW
BEGIN
SET NEW.CREATION_DATE = NOW();
END$$
...
USE `db_appdev_updated`$$
DROP TRIGGER IF EXISTS `db_appdev_updated`.`monthly_details_AFTER_INSERT` $$
USE `db_appdev_updated`$$
...
BEGIN
DECLARE comID INT;
SET comID = (SELECT CMI.COM_ID FROM MONTHLY_DETAILS MD JOIN COM_MONTHLY_INVENTORY CMI ON MD.COM_INVENTORY_ID = CMI.COM_INVENTORY_ID WHERE CMI.COM_INVENTORY_ID = 5 LIMIT 1);
...
USE `db_appdev_updated`$$
DROP TRIGGER IF EXISTS `db_appdev_updated`.`monthly_details_BEFORE_INSERT` $$
USE `db_appdev_updated`$$
CREATE DEFINER = CURRENT_USER TRIGGER `db_appdev_updated`.`monthly_details_BEFORE_INSERT` BEFORE INSERT ON `monthly_details` FOR EACH ROW
BEGIN
DECLARE sysCount DOUBLE;
DECLARE comID INT;
SET comID = (SELECT CMI.COM_ID FROM COM_MONTHLY_INVENTORY CMI WHERE CMI.COM_INVENTORY_ID = NEW.COM_INVENTORY_ID LIMIT 1);
SET sysCount = (SELECT CURRENT_QTY FROM COM_INVENTORY WHERE COM_ID = comID AND STOCK_ID = NEW.STOCK_ID);
SET NEW.DISCREPANCY_QTY = (NEW.ACTUAL_QTY - sysCount);
END$$
